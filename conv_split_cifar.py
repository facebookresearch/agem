...
def main():
    ...
    experiment_id = "SPLIT_CIFAR_%r_%r_%s_%s_%s_%r_%s-"%(EVAL_SINGLE_HEAD, TRAIN_SINGLE_EPOCH, args.imp_method, str(args.synap_stgth).replace('.', '_'), 
    ...
    if TRAIN_SINGLE_EPOCH:
        print('A10: {}'.format(acc_mean[-1,-1,:].mean()))
    else:
        print(exper_acc)
        print('A10: {}'.format(acc_mean[-1,:].mean()))
